...
[timer1SoundEnable, timer1SoundSec, timer1Volume,
 timer2SoundEnable, timer2SoundSec, timer2Volume].forEach(el=>{
...
function updateDisplay(){
    ...
    if(!timer1Running){
        ...
        if(timer1SoundEnable.checked && timer1Total === parseInt(timer1SoundSec.value)){
            playSound(1);
        }
    }
    else{
    ...
    if(!timer2Running){
        ...
        if(timer2SoundEnable.checked && timer2Total === parseInt(timer2SoundSec.value)){
            playSound(2);
        }
    }
    else{
...
function defaultBeep(timer){
    ...
    const volume = (timer===1) ? parseFloat(timer1Volume.value) : parseFloat(timer2Volume.value);
    const frequencies = (timer===1) ? [800,1000,1200] : [1000,1000,1000]; // 可分別設定兩個timer
